...
/**
 * ROVER SLAM PRO - MOBILE OPTIMIZED
 * Pins Used:
...
// ========= DASHBOARD HTML (Auto-Scaling & HUD Optimized) =========
...
<head>
    ...
    <style>
        ...
        canvas { background: #000; display: block; margin: 5px auto; width: 98vw; height: 55vh; border-radius: 8px; border: 1px solid #333; }
        .controls { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; padding: 12px; }
        button { background: #111; color: #00ff41; border: 1px solid #00ff41; padding: 18px; font-size: 14px; font-weight: bold; border-radius: 8px; transition: 0.2s; }
...
<body>
    <div class="hud-grid">
        ...
        <div class="hud-item">MISSION<span class="val" id="st">IDLE</span></div>
    </div>
    
    <div id="status-bar">SYSTEM ACTIVE | WAITING FOR COMMAND</div>
    <canvas id="map"></canvas>
    
    <div class="controls">
        <button id="mainBtn" onclick="toggle()">START MISSION</button>
        <button onclick="resetMap()">RESET MAP</button>
        <button class="stop" onclick="cmd('/stop')" style="grid-column: span 2;">EMERGENCY STOP</button>
    </div>
    ...
    <script>
        ...
        async function update() {
            try {
                ...
                document.getElementById('st').innerText = d.surveying ? 'RUNNING' : 'IDLE';
        ...
        function draw() {
            ...
            ctx.scale(scale, -scale); // Flip Y for math
...
void setup() {
    ...
    server.on("/data", []() {
        ...
        snprintf(json, sizeof(json), 
            "{\"x\":%.1f,\"y\":%.1f,\"ox\":%.1f,\"oy\":%.1f,\"heading\":%.1f,\"distance\":%.1f,\"isObs\":%s,\"surveying\":%s,\"mpu\":%s}",
            x, y, ox, oy, headingDeg, distanceCM, 
            obstacleDetected?"true":"false", surveying?"true":"false", mpuStatus?"true":"false");
...
void loop() {
    ...
    if (surveying && (millis() - lastLogicTick > 50)) {
        ...
        if (obstacleDetected) {
            stopMotors();
            applyMotors(LOW, HIGH, HIGH, LOW); // Turn Right
            delay(150); 
        } else {
            applyMotors(HIGH, LOW, HIGH, LOW); // Forward
            x += 1.1 * cos(rad);
            y += 1.1 * sin(rad);
        }
    } else if (!surveying) {
